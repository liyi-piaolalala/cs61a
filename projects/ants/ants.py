...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            tmp1=self
            tmp2=place.ant
            if tmp1.is_container and tmp1.can_contain(tmp2):
                tmp1.store_ant(tmp2)
                place.ant=tmp1
            elif tmp2.is_container and tmp2.can_contain(tmp1):
                tmp2.store_ant(tmp1)
                place.ant=tmp2
            else:
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if not self.already_doubled:
            self.damage*=2
            self.already_doubled=True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        tmp=self.place
        t=0
        while (tmp.bees==[] or t<self.lower_bound) and not tmp.is_hive and t<self.upper_bound :
            t+=1
            tmp=tmp.entrance
        if tmp.is_hive:
            return None
        else:
            return random_bee(tmp.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health<=amount:
            amount1=amount+self.damage
        tmp=self.place.bees[::]
        for x in tmp:
            Insect.reduce_health(x,amount1)
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if self.cooldown==0:
            if self.place.bees!=[]:
                tmp=random_bee(self.place.bees)
                Insect.reduce_health(tmp,tmp.health)
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.ant_contained!=None:
            self.ant_contained.action(gamestate)#默认取决于实例调用还是类方法调用
        # END Problem 8a
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while x :
            if x.ant:
                x.ant.double()
                if x.ant.is_container and x.ant.ant_contained!=None:
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 1
        "*** YOUR CODE HERE ***"#实例和类属性都是可以动态添加的
        if target is not None:
            if not hasattr(target,'slowbuff'):
                target.tmp=target.action
            target.slowbuff=5
            def slow_action(target,gamestate):
                if target.slowbuff>0:
                    if gamestate.time%2==1:
                        target.slowbuff-=1
                    else :    
                        target.tmp(gamestate)
                        target.slowbuff-=1
                else:
                    target.tmp(gamestate)
            target.action=lambda gamestate:slow_action(target,gamestate)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while  not tmp.is_hive:
            if tmp.ant!=self and tmp.ant!=None:
                result[tmp.ant]=distance
            for x in tmp.bees:
                result[x]=distance
            tmp=tmp.entrance
            distance+=1
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.scared and self.scarebuff>0:
            self.scare(1)
            self.scarebuff-=1
        else:
            if self.blocked():
    ...
    def scare(self, length):
        ...
        if self.scare:
            while length>0:
                if self.blocked():
                    self.sting(self.place.ant)
                elif not self.place.entrance.is_hive and self.health>0:
