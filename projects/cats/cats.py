...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if immutable_args not in cache:
            ...
            if limit>cache[immutable_args][1]:
                result=diff_function(typed,source,limit)
                cache[immutable_args]=(result,limit)
                return result
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    min,tmp=diff_function(typed_word,word_list[0],limit),word_list[0]
    for x in word_list:
        if diff_function(typed_word,x,limit)<min:
            min,tmp=diff_function(typed_word,x,limit),x
    if min>limit:
        return typed_word
    return tmp
...
def furry_fixes(typed, source, limit):
    ...
    if limit<0:
        ...
        return len(typed)-len(source)+furry_fixes(typed[:len(source):],source,limit-(len(typed)-len(source)))
    elif len(typed)<len(source):
        return len(source)-len(typed)+furry_fixes(typed,source[:len(typed):],limit-(len(source)-len(typed)))
    else:
        if typed[0]==source[0]:
...
def minimum_mewtations(typed, source, limit):
    ...
    if abs(len(source)-len(typed))>limit:
        ...
        return 1+limit
    elif typed in source:
        return len(source)-len(typed)
    elif source in typed:
        ...
        if source[0]==typed[0]:
           return minimum_mewtations(typed[1::],source[1::],limit-1)#因为f[i][j],f[i+1][j],f[i][j+1]之差不会超过1
        tmp=1+min(minimum_mewtations(typed[1::],source,limit-1),minimum_mewtations(typed[1::],source[1::],limit-1),minimum_mewtations(typed,source[1::],limit-1))
        return tmp
...
def final_diff(typed, source, limit,sign=0):
    """A diff function that takes in a string TYPED, a string SOURCE, and a number LIMIT.
    If you implement this function, it will be used."""
    if sign==0 :
        result=[final_diff(typed,source,limit,1)]
        for i in range(1,len(typed)):
            for j in range(i):
                tmp1=list(typed)
                tmp1[i],tmp1[j]=tmp1[j],tmp1[i]
                tmp1=''.join(tmp1)
                result+=[1+final_diff(tmp1,source,limit-1,1)]
        return min(result)
    if abs(len(source)-len(typed))>limit:
        return limit+1
    elif limit<0:
        ...
        if source[0]==typed[0]:
           return final_diff(typed[1::],source[1::],limit-1,1)
        tmp=1+min(final_diff(typed[1::],source,limit-1,1),final_diff(typed[1::],source[1::],limit-1,1),final_diff(typed,source[1::],limit-1,1))
        return tmp
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    count=0
    for x in range(min(len(typed),len(source))):
        if typed[x]==source[x]:
            count+=1
        else :
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[ x[i+1]-x[i] for i in range(len(x)-1)] for x in tpp]  # You may remove this line
    # END PROBLEM 9
