...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    p,o=player_score%10,opponent_score%100//10
    if p-o==0:
        return 1
    elif p-o>0:
        return 3*(p-o)
...
def make_averaged(original_function, times_called=1000):
    ...
    def f(*arg):
        s,t=0,0
        while times_called>t:
            s+=original_function(*arg)
            t+=1
        s=s/t
        return s
    return f
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    while t<10:
        t+=1
        tmpf=make_averaged(roll_dice,times_called)
        tmp1=tmpf(t,dice)#恶心，真恶心
        if tmp1>max:
            max=tmp1
            tmp=t
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    t=boar_brawl(score,opponent_score)
    if t>=threshold:
        return 0
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    s=sus_update(0,score,opponent_score)
