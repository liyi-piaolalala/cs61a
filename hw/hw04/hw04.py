def shuffle(s):
    ...
    if type(s)!=list:
        ...
        r=s
    def interleaves(s1,s2):
        if len(s1)==1:
            return s1+s2
        return [s1[0]]+[s2[0]]+interleaves(s1[1::],s2[1::])
    return interleaves(r[:len(s)//2:],r[len(s)//2::])
...
def balanced(m):
    ...
    if is_planet(m):
        return True
    elif total_mass(end(left(m)))*length(left(m))==total_mass(end(right(m)))*length(right(m)) and balanced(end(left(m))) and balanced(end(right(m))):
        return True
    return False
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        return max( [ max_path_sum(x) for x in branches(t) ])+label(t)
